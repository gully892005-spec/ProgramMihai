...
async function searchHotelsLive() {
  ...
  return hotels.slice(0,20).map((h,i) => {
    ...
    return {
      ...
      pricePerPerson:Math.round(perNight/State.adults), nights,
      ...
      offerUrl:"https://booking.com", transport:"avion", isLive:true,
    };
  });
}
...
function filterDemo() {
  ...
  if(State.dest){
    const filtered=r.filter(o=>
      o.dest.toLowerCase().includes(State.dest.toLowerCase())||
      o.country.toLowerCase().includes(State.dest.toLowerCase())
    );
