 ...
 *
 * ============================================================
 * CONFIGURARE API – înlocuiește valorile de mai jos cu cheile tale reale
 * ============================================================
 */
...
const API_CONFIG = {
  /**
   * BOOKING.COM via RapidAPI (hoteluri + mașini)
  ...
  /**
   * SKYSCANNER via RapidAPI (seat classes meta)
...
/* ============================================================
   FUNCȚII API – GOOGLE FLIGHTS via RapidAPI
   ============================================================ */

/**
 * Caută zboruri pe Google Flights via RapidAPI.
 * @param {Object} params - { from, to, adults, cabinClass }
 * @returns {Promise<Array>}
 */
async function fetchGoogleFlights(params) {
  if (!API_CONFIG.GOOGLE_FLIGHTS.ENABLED) return [];

  const { from = "OTP", to = "JFK", adults = 1, cabinClass = "ECONOMY" } = params;

  const url = new URL(API_CONFIG.GOOGLE_FLIGHTS.BASE_URL);
  url.searchParams.set("departure_id", from);
  url.searchParams.set("arrival_id", to);
  url.searchParams.set("travel_class", cabinClass);
  url.searchParams.set("adults", String(adults));
  ...
  try {
    const response = await fetch(url.toString(), {
      ...
      headers: {
        ...
        "x-rapidapi-host": API_CONFIG.GOOGLE_FLIGHTS.HOST,
        "x-rapidapi-key": API_CONFIG.GOOGLE_FLIGHTS.KEY,
      },
    });

    if (!response.ok) throw new Error(`Google Flights error: ${response.status}`);

    const data = await response.json();
    const flights = data?.data?.flights || data?.flights || [];

    return flights.slice(0, 6).map((f, idx) => ({
      id: `gf_${idx}_${Date.now()}`,
      title: `Zbor ${from} → ${to}`,
      destination: to,
      type: "city-break",
      transport: "avion",
      ...
      persons: adults,
      nights: 0,
      checkIn: f?.departure_date || "",
      checkOut: f?.arrival_date || "",
      image: "https://images.unsplash.com/photo-1436491865332-7a61a109cc05?w=600&q=80",
      imageAlt: "Zbor Google Flights",
      source: "Google Flights",
      sourceLogo: "✈️",
      badge: "city-break",
      badgeLabel: "✈️ ZBOR",
      room: `Clasa ${cabinClass.toLowerCase()}`,
      included: ["Bagaj de mână"],
      offerUrl: f?.booking_link || "https://flights.google.com",
      description: `Zbor ${from} → ${to}. Clasa ${cabinClass.toLowerCase()}, ${adults} adult(i).`,
    }));
  } catch (err) {
    console.warn("Google Flights API error:", err);
    return [];
  }
}

/* ============================================================
   FUNCȚII API – AIRBNB via RapidAPI
   ============================================================ */

/**
 * Caută proprietăți Airbnb după placeId.
 * @param {Object} params - { placeId, adults, currency }
 * @returns {Promise<Array>}
 */
async function fetchAirbnbProperties(params) {
  if (!API_CONFIG.AIRBNB.ENABLED) return [];
  ...
  const url = new URL(API_CONFIG.AIRBNB.BASE_URL);
  url.searchParams.set("placeId", placeId);
  url.searchParams.set("adults", String(adults));
  url.searchParams.set("guestFavorite", "false");
  url.searchParams.set("ib", "false");
  ...
  try {
    const response = await fetch(url.toString(), {
      ...
      headers: {
        "Content-Type": "application/json",
        "x-rapidapi-host": API_CONFIG.AIRBNB.HOST,
        "x-rapidapi-key": API_CONFIG.AIRBNB.KEY,
      },
    });

    if (!response.ok) throw new Error(`Airbnb error: ${response.status}`);

    const data = await response.json();
    const listings = data?.data?.list || data?.list || [];

    return listings.slice(0, 8).map((item, idx) => {
      ...
      return {
        id: `airbnb_${listing?.id || idx}_${Date.now()}`,
        title: sanitize(listing?.name || "Proprietate Airbnb"),
        destination: sanitize(listing?.city || "Destinație"),
        type: "city-break",
        transport: "all",
        stars: Math.round(listing?.avgRating || 4),
        rating: parseFloat(listing?.avgRating || "8.0"),
        pricePerPerson: Math.round(priceNum / Math.max(adults, 1)),
        persons: adults,
        nights: pricing?.nights || 1,
        checkIn: "",
        checkOut: "",
        image: listing?.pictureUrl?.large || listing?.pictureUrl?.medium || "https://images.unsplash.com/photo-1566073771259-6a8506099945?w=600&q=80",
        imageAlt: listing?.name || "Airbnb listing",
        source: "Airbnb",
        sourceLogo: "🏠",
        badge: "city-break",
        badgeLabel: "🏠 AIRBNB",
        room: listing?.roomType || "Întreg spațiul",
        included: ["Wi-Fi", "Bucătărie"],
        offerUrl: `https://airbnb.com/rooms/${listing?.id || ""}`,
        description: sanitize(listing?.name || "Cazare Airbnb") + `. Rating: ${listing?.avgRating || "N/A"}/5.`,
      };
    });
  } catch (err) {
    console.warn("Airbnb API error:", err);
    return [];
  }
}

/* ============================================================
   FUNCȚII API – CAR RENTALS via Booking.com RapidAPI
   ============================================================ */

/**
 * Caută mașini de închiriat via Booking.com RapidAPI.
 * @param {Object} params - { pickUpLat, pickUpLng, dropOffLat, dropOffLng, pickUpTime, dropOffTime, driverAge, currency }
 * @returns {Promise<Array>}
 */
async function fetchCarRentals(params) {
  if (!API_CONFIG.CAR_RENTALS.ENABLED) return [];
  ...
  const url = new URL(API_CONFIG.CAR_RENTALS.BASE_URL);
  url.searchParams.set("pick_up_latitude", String(pickUpLat));
  url.searchParams.set("pick_up_longitude", String(pickUpLng));
  url.searchParams.set("drop_off_latitude", String(dropOffLat));
  url.searchParams.set("drop_off_longitude", String(dropOffLng));
  url.searchParams.set("pick_up_time", pickUpTime);
  url.searchParams.set("drop_off_time", dropOffTime);
  url.searchParams.set("driver_age", String(driverAge));
  url.searchParams.set("currency_code", currency);
  url.searchParams.set("location", "RO");
  ...
  try {
    const response = await fetch(url.toString(), {
      ...
      headers: {
        "Content-Type": "application/json",
        "x-rapidapi-host": API_CONFIG.CAR_RENTALS.HOST,
        "x-rapidapi-key": API_CONFIG.CAR_RENTALS.KEY,
      },
    });

    if (!response.ok) throw new Error(`Car Rentals error: ${response.status}`);

    const data = await response.json();
    const cars = data?.data?.search_results || data?.search_results || [];

    return cars.slice(0, 6).map((car, idx) => ({
      id: `car_${idx}_${Date.now()}`,
      title: sanitize(car?.vehicle_info?.v_name || "Mașină de închiriat"),
      destination: "București, România",
      type: "city-break",
      transport: "masina",
      stars: 3,
      rating: parseFloat(car?.content?.reviews_from_user?.average_score || "7.5"),
      pricePerPerson: Math.round(parseFloat(car?.pricing_info?.price || 0)),
      persons: 1,
      nights: 1,
      checkIn: "",
      checkOut: "",
      image: car?.vehicle_info?.image_url || "https://images.unsplash.com/photo-1553440569-bcc63803a83d?w=600&q=80",
      imageAlt: car?.vehicle_info?.v_name || "Mașină",
      source: "Booking Cars",
      sourceLogo: "🚗",
      badge: "city-break",
      badgeLabel: "🚗 CAR RENTAL",
      room: `${car?.vehicle_info?.category || "Sedan"} · ${car?.vehicle_info?.transmission || "Manual"}`,
      included: ["Asigurare de bază", "Kilometraj nelimitat"],
      offerUrl: car?.pricing_info?.booking_url || "https://booking.com/cars",
      description: sanitize(`${car?.vehicle_info?.v_name || "Mașină"} – ${car?.vehicle_info?.category || ""}. Preț: ${car?.pricing_info?.price || "N/A"} ${currency}.`),
    }));
  } catch (err) {
    console.warn("Car Rentals API error:", err);
    return [];
  }
}

/* ============================================================
   FUNCȚII API – TRIPADVISOR Restaurante via RapidAPI
   ============================================================ */

/**
 * Caută restaurante pe TripAdvisor via RapidAPI.
 * @param {Object} params - { locationId }
 * @returns {Promise<Array>}
 */
async function fetchTripadvisorRestaurants(params) {
  if (!API_CONFIG.TRIPADVISOR.ENABLED) return [];

  const { locationId = "304554" } = params;

  const url = new URL(API_CONFIG.TRIPADVISOR.BASE_URL);
  ...
  try {
    const response = await fetch(url.toString(), {
      ...
      headers: {
        "Content-Type": "application/json",
        "x-rapidapi-host": API_CONFIG.TRIPADVISOR.HOST,
        "x-rapidapi-key": API_CONFIG.TRIPADVISOR.KEY,
      },
    });

    if (!response.ok) throw new Error(`TripAdvisor error: ${response.status}`);

    const data = await response.json();
    return data?.data?.data || data?.data || [];
  } catch (err) {
    console.warn("TripAdvisor API error:", err);
    return [];
  }
}

/* ============================================================
   FUNCȚII API – RYANAIR Calendar Disponibilitate via RapidAPI
   ============================================================ */

/**
 * Obține calendarul de disponibilitate Ryanair.
 * @param {Object} params - { origin, destination }
 * @returns {Promise<Object>}
 */
async function fetchRyanairCalendar(params) {
  if (!API_CONFIG.RYANAIR.ENABLED) return null;

  const { origin = "OTP", destination = "STN" } = params;

  const url = new URL(API_CONFIG.RYANAIR.BASE_URL);
  ...
  try {
    const response = await fetch(url.toString(), {
      ...
      headers: {
        "Content-Type": "application/json",
        "x-rapidapi-host": API_CONFIG.RYANAIR.HOST,
        "x-rapidapi-key": API_CONFIG.RYANAIR.KEY,
      },
    });

    if (!response.ok) throw new Error(`Ryanair error: ${response.status}`);

    return await response.json();
  } catch (err) {
    console.warn("Ryanair API error:", err);
    return null;
  }
}

/* ============================================================
   FUNCȚII API – AERODATABOX via RapidAPI
   ============================================================ */

/**
 * Verifică balanța subscripției AeroDataBox.
 * @returns {Promise<Object>}
 */
async function fetchAerodataboxBalance() {
  if (!API_CONFIG.FLIGHTS.ENABLED) return null;
  ...
  try {
    const response = await fetch("https://aerodatabox.p.rapidapi.com/subscriptions/balance", {
      ...
      headers: {
        ...
        "x-rapidapi-key": API_CONFIG.FLIGHTS.KEY,
      },
    });
    ...
    console.warn("AeroDataBox API error:", err);
    return null;
  }
}

/**
 * Caută zboruri via AeroDataBox.
 * @param {Object} params - { flightNumber, date }
 * @returns {Promise<Array>}
 */
async function fetchAerodataboxFlights(params) {
  if (!API_CONFIG.FLIGHTS.ENABLED) return [];

  const { flightNumber = "RO001", date } = params;
  const dateStr = date || new Date().toISOString().split("T")[0];

  try {
    const response = await fetch(`${API_CONFIG.FLIGHTS.BASE_URL}/number/${flightNumber}/${dateStr}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        "x-rapidapi-host": API_CONFIG.FLIGHTS.HOST,
    ...
    if (!response.ok) throw new Error(`AeroDataBox flights error: ${response.status}`);

    const data = await response.json();
    return data || [];
  } catch (err) {
    console.warn("AeroDataBox flights error:", err);
    return [];
  }
}

/* ============================================================
   FUNCȚII API – HOTELS.COM via RapidAPI
   ============================================================ */

/**
 * Obține detalii proprietate Hotels.com.
 * @param {Object} params - { hotelId, checkIn, checkOut, adults, currency }
 * @returns {Promise<Object|null>}
 */
async function fetchHotelsDetails(params) {
  if (!API_CONFIG.HOTELS.ENABLED) return null;
  ...
  const url = new URL(API_CONFIG.HOTELS.BASE_URL);
  url.searchParams.set("id", String(hotelId));
  url.searchParams.set("checkIn", checkIn);
  url.searchParams.set("checkOut", checkOut);
  url.searchParams.set("adults1", String(adults));
  url.searchParams.set("currency", currency);
  url.searchParams.set("locale", "ro_RO");
  ...
  try {
    const response = await fetch(url.toString(), {
      ...
      headers: {
        "Content-Type": "application/json",
        "x-rapidapi-host": API_CONFIG.HOTELS.HOST,
        "x-rapidapi-key": API_CONFIG.HOTELS.KEY,
      },
    });

    if (!response.ok) throw new Error(`Hotels.com error: ${response.status}`);

    return await response.json();
  } catch (err) {
    console.warn("Hotels.com API error:", err);
    return null;
  }
}
...
async function searchOffers(query) {
  ...
  try {
    ...
    if (API_CONFIG.GOOGLE_FLIGHTS.ENABLED) {
      apiCalls.push(
        fetchGoogleFlights({
          from: "OTP",
          to: parsed.destination ? parsed.destination.toUpperCase().slice(0, 3) : "JFK",
          adults: parsed.persons || 1,
          cabinClass: "ECONOMY",
        }).catch((err) => {
          console.warn("Google Flights API error:", err);
          return [];
    ...
    if (API_CONFIG.CAR_RENTALS.ENABLED && /mașin[aă]|auto|car|rent/i.test(query)) {
      apiCalls.push(
        fetchCarRentals({
          pickUpLat: 44.4268,
          pickUpLng: 26.1025,
...
function initEventListeners() {
  ...
  document.getElementById("reset-search").addEventListener("click", () => {
    ...
    AppState.filters = { type: "all", transport: "all", persons: 0, budgetMax: 5000 };
    syncFilterUI();
    hideAllStates();
  });
...
const DateState = {
  mode: "month",       // "month" | "range" | "flexible"
